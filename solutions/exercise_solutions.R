...
strat_mean <- long %>%
  ...
  summarise(pf_tot = sum(density, na.rm = TRUE), .groups = "drop") %>%
...
bonus <- long %>%
  mutate(depth = ifelse(str_detect(STRAT, "DEEP"), "DEEP", "SHALLOW")) %>%
  group_by(YEAR, REGION, depth, PRIMARY_SAMPLE_UNIT) %>%
  filter(density > 0) %>% 
  summarise(rich = n_distinct(SPECIES_CD), .groups = "drop") %>% 
  group_by(YEAR, REGION, depth) %>%
